-- BD Examen 2ª Evaluación 2025
--***********************************************************************************************************************
--NOTAS:
	-- 1. SOLAMENTE SE PUEDE TENER ABIERTO UN EDITOR DE TEXTOS Y SQLDEVELOPER
	-- 2. Solo se pueden hacer modificaciones en este fichero
	-- 3. Si la ejecución de algún sentencia falla, se deberá comentar
	-- 4. El fichero deberá funcionar correctamente. Si se produce el error en alguna línea, se dará por suspenso
	-- 5. Se entregará unicamente este archivo con las modificaciones que se hayan realizado
--***********************************************************************************************************************

--DROP TABLES
DROP TABLE ACTOR CONSTRAINT CASCADE;
DROP TABLE MIEMBRO_JURADO CONSTRAINT CASCADE;
DROP TABLE PAPEL CONSTRAINT CASCADE;
DROP TABLE ACTUACION CONSTRAINT CASCADE;
DROP TABLE FICHA_PUNTUACION CONSTRAINT CASCADE;


---- 0. Permite la introducción de la fecha de forma europea DD/MM/AAAA
...
INSERT INTO FICHA_PUNTUACION VALUES (8,3,2,'12345678C','22233344A');
...
-- d. Inserta un nuevo actor en la tabla ACTOR con los siguientes datos:
  ...
  INSERT INTO ACTOR(dni,nombre,edad) VALUES ('23456789B','Pedro Ruiz',30);
  
-- e. Inserta un nuevo miembro del jurado con nombre 'María López' y especialidad 'Cine'.
INSERT INTO MIEMBRO_JURADO (nombre, especialidad) VALUES ('María López','Cine');
...
INSERT INTO MIEMBRO_JURADO VALUES ('34567890C', 'María López','Actuación');

-- g. Inserta un nuevo papel Programador.
INSERT INTO PAPEL VALUES (8,'Programador');

-- h. Registra una actuación en la que el actor '45678123A' interpreta el papel '8' el 1 de marzo de 2025.
INSERT INTO ACTUACION VALUES (8,'45678123A', '01/03/25');
...
INSERT INTO ACTUACION VALUES (1,'23456789B', '24-02-25');

-- j. Inserta una nueva puntuación en la que el jurado '55566677D' puntúa a '45678123A' en el papel '8' con nota 1.
INSERT INTO FICHA_PUNTUACION VALUES(9,1,8,'45678123A','55566677D');
...
INSERT INTO FICHA_PUNTUACION VALUES(10,8,1,'23456789B','34567890C');
...
SELECT actor.nombre
FROM actor INNER JOIN actuacion ON actor.dni = actuacion.dni_act
WHERE cod_pap = (SELECT cod_pap
				 FROM papel
...
SELECT actor.nombre, papel.descripcion
FROM actor INNER JOIN actuacion ON actor.dni = actuacion.dni_act
			INNER JOIN papel ON actuacion.cod_pap = papel.cod_pap
WHERE fecha BETWEEN '01/01/17' AND '31/01/17'

-- c. Obtener la edad media de los actores que han realizado actuaciones
SELECT AVG(edad)
FROM actor 
WHERE actor.dni IN (SELECT dni_act
                    FROM actuacion)

-- d. Obtener la descripción de los papeles que han sido puntuados
SELECT papel.descripcion
FROM papel
WHERE cod_pap IN (SELECT cod_papel
                  FROM ficha_puntuacion)

-- e. Obtener el nombre de los actores que han sacado la nota máxima
SELECT actor.nombre
FROM actor INNER JOIN ficha_puntuacion ON actor.dni = ficha_puntuacion.dni_act
WHERE nota = (SELECT MAX(nota)
              FROM ficha_puntuacion)


-- f. Muestra el dni y nombre de los miembros del jurado, junto con la puntuación promedio que han dado, 
--pero solo para los jurados que han puntuado más de 3 actuaciones. El resultado debe estar ordenado de mayor a menor por la puntuación promedio.
...
SELECT actor.nombre, papel.descripcion
FROM actor, atuacion, papel
WHERE actor.dni IN actuacion.dni_act AND
  actuacion.cod_pap IN papel.cod_pap
...
SELECT actor.nombre, actor.dni, actor.edad
FROM actor INNER JOIN ficha_puntuacion ON actor.dni = ficha_puntuacion.dni_act
WHERE nota = (SELECT MAX(nota)
              FROM ficha_puntuacion)
...
DELETE FROM actuacion WHERE actuaciones = '' AND fecha < '06/03/25';

-- i. Elimina la tabla actoresParaJubilar
DROP TABLE actoresParaJubilar CONSTRAINT CASCADE;